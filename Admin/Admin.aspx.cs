...
namespace CrudPractice.Admin
{
    public partial class Admin : System.Web.UI.Page
    {
        ...
        protected void gvEmployeeList_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string query = @"update tbl_Employee set EmployeName = @EmployeName, EmployeePhone= @EmployeePhone, EmployeeEmail=@EmployeeEmail,
            Address = @Address, DepartmentId = @DepartmentId where EmployeeID = @id";
            SqlCommand cmd = new SqlCommand(query,con);
            ...
            cmd.Parameters.AddWithValue("@DepartmentId", Convert.ToInt32((gvEmployeeList.Rows[e.RowIndex].FindControl("txtEmpDepartment") as TextBox).Text)) ;
            cmd.Parameters.AddWithValue("@id", Convert.ToInt32(gvEmployeeList.DataKeys[e.RowIndex].Value.ToString()));
            cmd.ExecuteNonQuery();
